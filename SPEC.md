...
9. **Рестарт приложения.** При перезапуске App, если `isRegistered == "approved"`:
   - REGISTER **не отправляется** — бэк узнаёт remote по `src_hash` (Identity отправителя)
   - App шлёт `REQ(m)`, `REQ(a)`, `REQ(d)` для получения актуальных секций
   - Бэк отвечает `CONFIG` → App отображает Dashboard
...
| 7  | FORBIDDEN | HA → App       | Доступ запрещён (remote отозван или не одобрен) |
| 8  | REQ       | App → HA       | Запрос секции конфигурации                  |
| 9  | REGISTER  | App → HA       | Запрос регистрации нового remote'а          |

Зарезервировано (на будущее): `tp=1, 10+`.
...
Поля внутри элемента `s[i]` зависят от типа устройства (см. раздел 9).
...
Подробное содержание секций — раздел 8.
...
Поля кроме `tp` и `id` — зависят от типа устройства (см. раздел 9).
...
Бэк отвечает PONG с актуальными хешами.
При расхождении клиент шлёт REQ для изменившихся секций.
...
Wire: `{0: 6, 2: {m: "hash", a: "hash", d: "hash"}}`

### 6.7. tp=6 PONG (HA → App)

Бэк отвечает PONG с актуальными хешами секций.
...
При `section="d"` бэк также шлёт `tp=2 STATUS` snapshot всех устройств.
...
Сервер отправляет этот пакет в ответ на CMD, PING или REQ от remote,
который не находится в списке одобренных (отозван или ещё не одобрен).
...
Клиент при получении tp=7 обязан:
- Сбросить локальное состояние регистрации
- Перейти на экран онбординга
- Не повторять запросы до повторной регистрации

---

## 7. Каналы связи

### 7.1. Определение активного канала (getActiveChannel)

| TCP online? | WiFi connected? | Результат |
|-------------|-----------------|-----------|
| да          | да              | `"WiFi"`  |
| да          | нет             | `"4G"`    |
| нет         | —, BLE online   | `"BLE"`   |
...
### 11.1. Что обеспечивается транспортом
...
### 12.1. Что меняется по сравнению с v0.2
...
## 15. Что осталось вне SPEC (бэклог)
...
1. **PING/PONG синхронизация — РЕАЛИЗОВАТЬ:**
   - Клиент шлёт PING с локальными хешами каждые 30 с
   - При расхождении → REQ для изменившихся секций
   - Watchdog: если PONG не приходил 30 с → офлайн → retry REQ
